...
@task_decorator(
    "RunCommunityRagFlowTriggeredJob",
    human_name="Run Community RagFlow Triggered Job",
    short_description="Run a triggered job for Community RagFlow from a scenario template"
)
class RunCommunityRagFlowTriggeredJob(Task):
    """
    Task to run a triggered job for Community RagFlow using a scenario template.
    ...
    config_specs = ConfigSpecs({
        "tag_key": StrParam(
                human_name="Tag key",
                short_description="Tag key to identify the scenario template (e.g., community_send_to_rag_template)",
                optional=False
            ),
        "tag_value": StrParam(
                human_name="Tag value",
                short_description="Tag value to identify the scenario template (e.g., True)",
                optional=False
            ),
    ...
    def run(self, params: ConfigParams, inputs) -> dict:
        ...
        create_job_dto = CreateTriggeredJobFromTemplateDTO(
            ...
            name="Community RagFlow Job",
            description="Scheduled job to download documentation and push to RAGflow for community bricks",
