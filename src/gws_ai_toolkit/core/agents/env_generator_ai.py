...
def _generate_env_file(
    ...
    if not api_key:
        raise ValueError(
            "OPENAI_API_KEY environment variable is not set. Please set it before running this task."
        )
    ...
    if not generated_content:
        raise Exception(
            "No environment file content was generated by the AI agent, please check the logs for details."
        )

    task.log_success_message(
        f"Environment file generated successfully with {len(generated_content)} characters"
    )
...
class PipEnvGeneratorAi(Task):
    ...
    output_specs = OutputSpecs(
        {
            "env_file": OutputSpec(
                File, human_name="Pipfile", short_description="Generated Pipfile for pipenv"
            )
        }
    )
