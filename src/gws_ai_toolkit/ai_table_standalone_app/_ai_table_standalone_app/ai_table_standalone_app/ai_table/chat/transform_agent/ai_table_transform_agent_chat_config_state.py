...
from .ai_table_transform_agent_chat_config import AiTableTransformAgentChatConfig


class AiTableTransformAgentChatConfigState(rx.State):
    """State management for AI Table Transform Agent configuration interface.

    This state class manages the configuration form for AI Table Transform Agent functionality,
    handling user interactions, form validation, and configuration persistence.

    Note:
        This configuration allows full customization of model and temperature
        for flexible transform agent behavior.
    """

    def get_config_section_name(self) -> str:
        """Get the configuration section name for AI Table Transform Agent

        Returns:
            str: 'ai_table_transform_agent_page' configuration section name
        """
        return 'ai_table_transform_agent_page'

    async def get_config(self) -> AiTableTransformAgentChatConfig:
        ...
        return cast(AiTableTransformAgentChatConfig, config)
    ...
    async def handle_config_save(self, new_config: AiTableTransformAgentChatConfig):
        """Save the configuration and show appropriate feedback

        Args:
            new_config (AiTableTransformAgentChatConfig): The new configuration to save
    ...
    async def handle_config_form_submit(self, form_data: dict):
        """Handle the AI Table Transform Agent configuration form submission"""
        ...
        new_config = AiTableTransformAgentChatConfig(
            model=new_model,
