...
def _unsync_pending() -> rx.Component:
    ...
    return rx.vstack(
        ...
        rx.hstack(
            ...
            rx.button("Cancel", size="3", on_click=UnsyncAllResourcesDialogState.close_dialog),
        ),
    )
...
def _unsync_running() -> rx.Component:
    ...
    return rx.vstack(
        ...
        rx.hstack(
            rx.progress(
                value=UnsyncAllResourcesDialogState.get_resource_unsync_progress_percent, flex="1"
            ),
            rx.text(
...
def _unsync_done() -> rx.Component:
    ...
    return rx.vstack(
        ...
        rx.foreach(
            UnsyncAllResourcesDialogState.unsync_errors, lambda error: rx.text(error, size="3")
        ),
...
def unsync_all_resources_dialog() -> rx.Component:
    ...
    return rx.vstack(
        ...
        rx.cond(
            UnsyncAllResourcesDialogState.has_loaded_resources_to_unsync,
            _dialog_content(),
            rx.hstack(
                rx.spinner(),
