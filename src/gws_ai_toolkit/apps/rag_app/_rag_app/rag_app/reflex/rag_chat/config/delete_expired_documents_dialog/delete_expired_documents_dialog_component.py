...
from .delete_expired_documents_dialog_state import DeleteExpiredDocumentsDialogState


def _delete_pending() -> rx.Component:
    """Content for the delete pending state."""
    return rx.cond(
        DeleteExpiredDocumentsDialogState.count_documents_to_delete > 0,
        rx.vstack(
            rx.text(
            ...
            rx.text(
                "Those documents were deleted from DataHub and are still synced with the RAG platform.",
                size="2",
                color="gray.600",
            ),
            rx.accordion.root(
                rx.accordion.item(
                    header="Show documents to delete",
                    content=rx.vstack(
                        rx.foreach(
                            DeleteExpiredDocumentsDialogState.documents_info,
                            lambda document: rx.text(f"• {document}", size="2"),
                        ),
                        padding_top="2",
                    ),
                ),
            ...
            rx.hstack(
                ...
                rx.button(
                    "Cancel", size="3", on_click=DeleteExpiredDocumentsDialogState.close_dialog
                ),
                spacing="2",
            ),
            spacing="3",
...
def _delete_running() -> rx.Component:
    ...
    return rx.vstack(
        ...
        rx.hstack(
            rx.progress(
                value=DeleteExpiredDocumentsDialogState.get_documents_delete_progress_percent,
                flex="1",
            ),
            rx.text(
...
def _delete_done() -> rx.Component:
    ...
    return rx.vstack(
        ...
        rx.foreach(
            DeleteExpiredDocumentsDialogState.delete_errors,
            lambda error: rx.text(error, size="2", color="red.600"),
        ),
        rx.button("Close", size="3", on_click=DeleteExpiredDocumentsDialogState.close_dialog),
        spacing="3",
    )
...
def delete_expired_documents_dialog() -> rx.Component:
    ...
    return rx.vstack(
        ...
        rx.cond(
            DeleteExpiredDocumentsDialogState.has_loaded_documents_to_delete,
            _dialog_content(),
            rx.hstack(
                rx.spinner(),
