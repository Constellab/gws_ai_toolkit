...
def history_config_dialog(state: ConversationChatStateBase) -> rx.Component:
    ...
    has_config_dialog = (
        hasattr(state, "show_config_dialog")
        and hasattr(state, "config_json_string")
        and hasattr(state, "close_config_dialog")
    )
    ...
    return rx.dialog.root(
        rx.dialog.content(
            rx.dialog.title("Conversation Configuration", size="5", margin_bottom="16px"),
            ...
            rx.hstack(
                rx.dialog.close(
                    rx.button("Close", variant="outline", on_click=state.close_config_dialog),
                ),
                justify="end",
                width="100%",
            ),
